...
int main()
{
	...
	cout << "--------------------Candy Crush--------------------------" << endl
		 << "To Pause/Save Game enter Row and Column to be -1,-1" << endl;
	system("Pause");
	ClearScreen();
...
int userinput(int grid[][gsize + 3], int &i)
{
	...
	if (!((row2 == (row1 - 1) && col1 == col2) || (row2 == (row1 + 1) && col1 == col2) || (row1 == row2 && col2 == (col1 - 1)) || (row1 == row2 && col2 == (col1 + 1))))
	{
...
void plainwrap(int grid[][gsize + 3])
{
	...
	for (int i = 0; i < gsize; i++)
	{
		for (int j = 0; j < gsize; j++)
		{
			...
			if (((grid[i][j] <= 15 && grid[i][j] >= 11) || (grid[i + 1][j] <= 15 && grid[i + 1][j] >= 11) || (grid[i + 2][j] <= 15 && grid[i + 2][j] >= 11)) && (c1 == c2) && (c2 == c3)) // vertically check 1 candy is a wrapped and all same colors
			{
				rstart = i;
				cstart = j;
				poptype = 'r';
			...
			if (((grid[i][j] <= 15 && grid[i][j] >= 11) || (grid[i][j + 1] <= 15 && grid[i][j + 1] >= 11) || (grid[i][j + 2] <= 15 && grid[i][j + 2] >= 11)) && (c1 == c2) && (c2 == c3)) // horizontal check for same color and 1 wrapped
			{
